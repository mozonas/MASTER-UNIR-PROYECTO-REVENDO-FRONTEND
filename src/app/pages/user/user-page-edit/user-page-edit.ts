...
export class UserPageEditComponent implements OnInit {
  ...
  onGuardar(): void {
    ...
    const usuarioActualizado: Partial<Usuario> & { password?: string; direccion?: string } = {
      ...this.editForm.value,
      password: (this.currentUser as any).password || '' // Mantiene la clave intacta requerida por el backend
    };
    ...
    this.userService.updatePerfilUsuario(this.currentUser.id, usuarioActualizado).subscribe({
      next: (response) => {
        ...
        // Redirigimos de vuelta a la vista de información de perfil del usuario
